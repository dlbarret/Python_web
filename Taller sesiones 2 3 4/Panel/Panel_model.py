...
# Crear un objeto de la clase PVModel
pv = PVModel(4, 3) ##4 paneles en serie y 3 en paralelo, permite realizar cálculos y operaciones definidas en la clase PVModel

# Generar datos aleatorios para G (irradiancia) y T (temperatura)
datos = []


for _ in range(10):
    G = random.uniform(800, 1200)  # Irradiancia entre 800 y 1200 W/m²
    T = random.uniform(273 + 15, 273 + 35)  # Temperatura entre 15 y 35 grados Celsius

    # Calcular el modelo PV
    _, Vmpp, Impp, P_max = pv.modelo_pv(G, T)
    datos.append((G, T, Vmpp, Impp, P_max))


# Imprimir los datos generados
for i, (G, T, Vmpp, Impp, P_max) in enumerate(datos):
    print(f"Dato {i+1}: G = {G:.2f} W/m², T = {T-273:.2f} °C, Vmp = {Vmpp:.2f} V, Imp = {Impp:.2f} A, Pmax = {P_max:.2f} W")

###Paso 2: Crear Base de Datos SQLite y Guardar Datos
##se crear una base de datos SQLite y guardar los 10 conjuntos de datos.
...
cursor.executemany('INSERT INTO pv_data (G, T, Vmp, Imp, Pmax) VALUES (?, ?, ?, ?, ?)', datos)
conn.commit()
...
##Paso 3: Configurar Aplicación Flask y Gráfica
##Vamos a crear una aplicación Flask que muestre una gráfica de Vmp vs Imp y Pmax.

###Instala Flask y Matplotlib si aún no lo has hecho: